...
function Sidebar({ isSidebarOpen, toggleSidebar }) {
  ...
  const { defaultload, organizationname } = useGlobalSyncupContext()
  ...
  const menuItems = [
    ...
    },
    {
      ...
      icon: <LuBarChart className="w-6 h-6" />,
    },
    {
      text: "Releases",
    ...
    const fetchBoards = async () => {
      if (boardid.id !== undefined) {
        try {
          ...
          toast.error("Error while fetching board name")
        }
        ...
        if (sortedBoards.length > 0) {
          setboardname(sortedBoards[0].name)
        } else {
              ...
              <BreadcrumbItem>
                {isSidebarOpen && (
                  <div className="text-base">
                    <Tooltip
                      content={
                        boardid.organization.length > 25 ? (
                          <>
                            {boardid.organization
                              .match(/.{1,25}/g)
                              .map((line, index) => (
                    ...
                    >
                      <span>
                        {boardid.organization.length > 7
                          ? `${boardid.organization.substring(0, 7)}...`
                          : boardid.organization}
                      </span>
                    </Tooltip>
                  </div>
